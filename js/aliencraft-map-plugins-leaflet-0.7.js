        ...
        kml: function(doc) {
            ...
            function getPlacemark(root) {
                ...
                if (lineStyle) {
                    ...
                    if (!isNaN(opacity)) properties['strokeopacity'] = opacity;
                    if (!isNaN(width)) properties['strokewidth'] = width;
                }
                if (polyStyle) {
                    ...
                    if (outline) properties['strokeopacity'] = outline === '1' ? properties['strokeopacity'] || 1 : 0;
                }
                if (extendedData) {
            ...
            function getLineStyle(extensions) {
                ...
                if (extensions) {
                    ...
                    if (lineStyle) {
                        ...
                        if (!isNaN(opacity)) style['strokeopacity'] = opacity;
                        // GPX width is in mm, convert to px with 96 px per inch
                        if (!isNaN(width)) style['strokewidth'] = width * 96 / 25.4;
                    }
                }
                return style;
            }
...
/////////////////////////////L.Control.EasyBar  L.Control.Button/////////////////
...
L.Control.Search = L.Control.extend({
	...
	options: {
		...
		position: 'topleft',
		hideMarkerOnCollapse: true,    //remove circle and marker on search control collapsed
...
onRemove: function(map) {
	...
	_handleKeypress: function (e) {	//run _input keyup event
		...
		switch(e.keyCode)
		{
			...
			case  8://Backspace
				...
				this.fire("searchterm:deleted");
				
			
			break;
			
				case  67://Backspace in mobile
	...
	_fillRecordsCache: function() {
		...
		if(this.options.layer)
		{
			...
			this._records = Object.keys(records).length;
			
			
//console.log("records",records);
var recordsisEmpty = Object.keys(records).length === 0;
if (recordsisEmpty){
	//console.log(" no records",records);
	this.fire('search:gavenorecords');
	...
	_handleSubmit: function() {	//button and tooltip click and enter submit
		...
		else
		{
			if(this._input.value === ''){	//hide _input only
				this.collapse();
				this.fire('search:locationsubmittedbutno');
			}
			else
			{
...
this._stopUnspider = false; //to evala
    ...
    p['circleSpiralSwitchover'] = 89;

    p['circleFootSeparation'] = 25;

    p['circleStartAngle'] = twoPi / 4;

    p['spiralFootSeparation'] = 28;

    p['spiralLengthStart'] = 2;
    ...
    p.spiderfy = function(markerData, nonNearbyMarkers) {
      ...
      spiderfiedMarkers = (function() {
        ...
        // marker.setZIndexOffset(1000000); /*to evgala*/
		 if (marker instanceof L.CircleMarker)  /*to evala*/
         {this._stopUnspider = true;} /*to evala*/
   else {marker.setZIndexOffset(1000000);}/*to evala*/
    ...
    p['unspiderfy'] = function(markerNotToMove) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (marker['_omsData'] != null) {
          ...
          //marker.setZIndexOffset(0); /*to evgala*/
		   if (!marker instanceof L.CircleMarker) { marker.setZIndexOffset(0); }/*to evala*/
...
L.Control.Button=L.Control.extend({includes:L.Mixin.Events,options:{position:"topright"},initialize:function(t,o){L.setOptions(this,o);var i=null;if(t instanceof HTMLElement){i=t;try{i.parentNode.removeChild(i)}catch(t){}}else{if("string"!=typeof t)throw new Error("L.Control.Button: failed to initialize, label must either be text or a dom element");(i=L.DomUtil.create("button",this.options.className)).innerHTML=t}return L.DomUtil.addClass(i,this.options.position),this._container=i,this},isToggled:function(){return L.DomUtil.hasClass(this._container,this.options.toggleButton)},_fireClick:function(t){if(this.fire("click"),this.options.toggleButton){this._container;this.isToggled()?L.DomUtil.removeClass(this._container,this.options.toggleButton):L.DomUtil.addClass(this._container,this.options.toggleButton)}},onAdd:function(t){if(this._container){L.DomEvent.on(this._container,"click",this._fireClick,this);var o=L.DomEvent.stopPropagation;L.DomEvent.on(this._container,"mousedown",o).on(this._container,"touchstart",o).on(this._container,"dblclick",o).on(this._container,"mousewheel",o).on(this._container,"MozMozMousePixelScroll",o),this.fire("load"),this._map=t}return this._container},onRemove:function(t){return this._container&&this._map&&(L.DomEvent.off(this._container,"click",this._fireClick,this),L.DomEvent.off(this._container,"mousedown",stop).off(this._container,"touchstart",stop).off(this._container,"dblclick",stop).off(this._container,"mousewheel",stop).off(this._container,"MozMozMousePixelScroll",stop),this.fire("unload"),this._map=null),this}}),L.control.button=function(t,o){return new L.Control.Button(t,o)};
...
function simpleheat(canvas) {
    ...
    this._ctx = canvas.getContext('2d', {willReadFrequently :true, antialias:true, powerPreference: "high-performance", xrCompatible: true, depth:true});
...
simpleheat.prototype = {
    ...
    radius: function (r, blur) {
        ...
        var circle = this._circle = this._createCanvas(),
            ctx = circle.getContext('2d',{willReadFrequently :true, antialias:true, powerPreference: "high-performance", xrCompatible: true, depth:true}),
            r2 = this._r = r + blur;
    ...
    gradient: function (grad) {
        ...
        var canvas = this._createCanvas(),
            ctx = canvas.getContext('2d',{willReadFrequently :true, antialias:true, powerPreference: "high-performance", xrCompatible: true, depth:true}),
